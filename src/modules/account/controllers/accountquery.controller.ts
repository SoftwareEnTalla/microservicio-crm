...
import { AccountDto } from "../dtos/all-dto";
...
@Controller("accounts/query")
export class AccountQueryController {
  #logger = new Logger(AccountQueryController.name);

  constructor(private readonly service: AccountQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all account with optional pagination" })
  @ApiResponse({ status: 200, type: AccountsResponse })
  @ApiQuery({ name: "options", required: false, type: AccountDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Account>    
  ): Promise<AccountsResponse<Account>> {
    try {
     
      const accounts = await this.service.findAll(options);
      logger.info("Retrieving all account");
      return accounts;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find account by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter account', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: AccountsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Account no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find accounts with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Account>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all accounts" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count accounts with conditions" })
  @ApiResponse({ status: 200, type: AccountsResponse<Account> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Accounts no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one account with conditions" })
  ...
  @ApiOperation({ summary: "Find one account or return error" })
  @ApiResponse({ status: 200, type: AccountResponse<Account> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    @Query() all: Record<string, any> = {}
  ): Promise<AccountResponse<Account> | Error> {
    try {
      const where: Record<string, any> = parseWhereParam(all);
      const entity = await this.service.findOne(where);

      if (!entity) {
        return new NotFoundException("Entidad Account no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the account to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<AccountResponse<Account>> {
    try {
      const account = await this.service.findOne({ where: { id } });
      if (!account) {
        throw new NotFoundException(
          "Account no encontrado para el id solicitado"
        );
      }
      return account;
    } catch (error) {

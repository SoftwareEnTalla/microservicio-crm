...
import { logger } from '@core/logs/logger';
  ...
  export class ProviderRepository {
    ...
    async findAll(options?: FindManyOptions<Provider>): Promise<Provider[]> {
      logger.info('Ready to findAll Provider on repository:',options);
    ...
    async findById(id: string): Promise<Provider | null> {
      const tmp: FindOptionsWhere<Provider> = { id } as FindOptionsWhere<Provider>;
      logger.info('Ready to findById Provider on repository:',tmp);
    ...
    async findByField(
      ...
      logger.info('Ready to findByField Provider on repository:',options);
    ...
    async count(): Promise<number> {
      logger.info('Ready to count Provider on repository...');
      let result= this.repository.count();
      logger.info('Was counted  instances of Provider on repository:');
      return result;
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[Provider[], number]> {
      logger.info('Ready to findByField Provider on repository:',where);
    ...
    async findOne(options?: Record<string, any>): Promise<Provider | null> {
      if (!options || Object.keys(options).length === 0) {
        logger.warn('No conditions provided for finding Provider.');
        return null;
      }
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) Provider:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<Provider> = options as FindOptionsWhere<Provider>;
      logger.info('Ready to findOneBy Provider on repository:', tmp);
      return this.repository.findOneBy(tmp);
    }
    ...
    async findOneOrFail(where?: Record<string, any>): Promise<Provider> {
      logger.info('Ready to findOneOrFail Provider on repository:',where);
    ...
    async update(
        ...
        logger.info('Updated instance of Provider with id:  was finded on repository:', instance);

...
@ApiTags("Provider Query")
@UseGuards(ProviderAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("providers/query")
export class ProviderQueryController {
  #logger = new Logger(ProviderQueryController.name);

  constructor(private readonly service: ProviderQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all provider with optional pagination" })
  @ApiResponse({ status: 200, type: ProvidersResponse })
  @ApiQuery({ name: "options", required: false, type: ProviderDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Provider>    
  ): Promise<ProvidersResponse<Provider>> {
    try {
     
      const providers = await this.service.findAll(options);
      logger.info("Retrieving all provider");
      return providers;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find provider by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter provider', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ProvidersResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Provider no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find providers with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Provider>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all providers" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count providers with conditions" })
  @ApiResponse({ status: 200, type: ProvidersResponse<Provider> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Providers no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one provider with conditions" })
  ...
  @ApiOperation({ summary: "Find one provider or return error" })
  @ApiResponse({ status: 200, type: ProviderResponse<Provider> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Provider no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the provider to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ProviderResponse<Provider>> {
    try {
      const provider = await this.service.findOne({ where: { id } });
      if (!provider) {
        throw new NotFoundException(
          "Provider no encontrado para el id solicitado"
        );
      }
      return provider;
    } catch (error) {

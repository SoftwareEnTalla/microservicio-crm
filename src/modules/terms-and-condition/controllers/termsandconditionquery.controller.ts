...
import { TermsAndConditionDto } from "../dtos/all-dto";
...
@ApiTags("TermsAndCondition Query")
@UseGuards(TermsAndConditionAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("termsandconditions/query")
export class TermsAndConditionQueryController {
  #logger = new Logger(TermsAndConditionQueryController.name);

  constructor(private readonly service: TermsAndConditionQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all termsandcondition with optional pagination" })
  @ApiResponse({ status: 200, type: TermsAndConditionsResponse })
  @ApiQuery({ name: "options", required: false, type: TermsAndConditionDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<TermsAndCondition>    
  ): Promise<TermsAndConditionsResponse<TermsAndCondition>> {
    try {
     
      const termsandconditions = await this.service.findAll(options);
      logger.info("Retrieving all termsandcondition");
  ...
  @ApiOperation({ summary: "Find termsandcondition by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter termsandcondition', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: TermsAndConditionsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "TermsAndCondition no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find termsandconditions with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<TermsAndCondition>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all termsandconditions" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count termsandconditions with conditions" })
  @ApiResponse({ status: 200, type: TermsAndConditionsResponse<TermsAndCondition> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one termsandcondition with conditions" })
  ...
  @ApiOperation({ summary: "Find one termsandcondition or return error" })
  @ApiResponse({ status: 200, type: TermsAndConditionResponse<TermsAndCondition> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get termsandcondition by ID" })
  @ApiResponse({ status: 200, type: TermsAndConditionResponse<TermsAndCondition> })
  @ApiResponse({ status: 404, description: "TermsAndCondition not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the termsandcondition to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<TermsAndConditionResponse<TermsAndCondition>> {
    try {
      const termsandcondition = await this.service.findOne({ where: { id } });

...
import { ContractStatusDto } from "../dtos/all-dto";
...
@ApiTags("ContractStatus Query")
@UseGuards(ContractStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("contractstatuss/query")
export class ContractStatusQueryController {
  #logger = new Logger(ContractStatusQueryController.name);

  constructor(private readonly service: ContractStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all contractstatus with optional pagination" })
  @ApiResponse({ status: 200, type: ContractStatussResponse })
  @ApiQuery({ name: "options", required: false, type: ContractStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ContractStatus>    
  ): Promise<ContractStatussResponse<ContractStatus>> {
    try {
     
      const contractstatuss = await this.service.findAll(options);
      logger.info("Retrieving all contractstatus");
  ...
  @ApiOperation({ summary: "Find contractstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter contractstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ContractStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ContractStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find contractstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ContractStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all contractstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count contractstatuss with conditions" })
  @ApiResponse({ status: 200, type: ContractStatussResponse<ContractStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades ContractStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one contractstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one contractstatus or return error" })
  @ApiResponse({ status: 200, type: ContractStatusResponse<ContractStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad ContractStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get contractstatus by ID" })
  @ApiResponse({ status: 200, type: ContractStatusResponse<ContractStatus> })
  @ApiResponse({ status: 404, description: "ContractStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the contractstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ContractStatusResponse<ContractStatus>> {
    try {
      const contractstatus = await this.service.findOne({ where: { id } });
      if (!contractstatus) {
        throw new NotFoundException(
          "ContractStatus no encontrado para el id solicitado"
        );
      }

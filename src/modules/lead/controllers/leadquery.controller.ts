...
@Controller("leads/query")
export class LeadQueryController {
  #logger = new Logger(LeadQueryController.name);

  constructor(private readonly service: LeadQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all lead with optional pagination" })
  @ApiResponse({ status: 200, type: LeadsResponse })
  @ApiQuery({ name: "options", required: false, type: LeadDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Lead>    
  ): Promise<LeadsResponse<Lead>> {
    try {
     
      const leads = await this.service.findAll(options);
  ...
  @ApiOperation({ summary: "Find lead by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter lead', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: LeadsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Lead no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find leads with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Lead>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all leads" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count leads with conditions" })
  @ApiResponse({ status: 200, type: LeadsResponse<Lead> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Leads no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one lead with conditions" })
  ...
  @ApiOperation({ summary: "Find one lead or return error" })
  @ApiResponse({ status: 200, type: LeadResponse<Lead> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Lead no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the lead to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<LeadResponse<Lead>> {
    try {
      const lead = await this.service.findOne({ where: { id } });
      if (!lead) {
        throw new NotFoundException(
          "Lead no encontrado para el id solicitado"
        );
      }
      return lead;
    } catch (error) {

...
@ApiTags("CrmClientProfileStatus Query")
@UseGuards(CrmClientProfileStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("crmclientprofilestatuss/query")
export class CrmClientProfileStatusQueryController {
  #logger = new Logger(CrmClientProfileStatusQueryController.name);

  constructor(private readonly service: CrmClientProfileStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all crmclientprofilestatus with optional pagination" })
  @ApiResponse({ status: 200, type: CrmClientProfileStatussResponse })
  @ApiQuery({ name: "options", required: false, type: CrmClientProfileStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<CrmClientProfileStatus>    
  ): Promise<CrmClientProfileStatussResponse<CrmClientProfileStatus>> {
    try {
     
      const crmclientprofilestatuss = await this.service.findAll(options);
      logger.info("Retrieving all crmclientprofilestatus");
  ...
  @ApiOperation({ summary: "Find crmclientprofilestatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter crmclientprofilestatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CrmClientProfileStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "CrmClientProfileStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find crmclientprofilestatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<CrmClientProfileStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all crmclientprofilestatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count crmclientprofilestatuss with conditions" })
  @ApiResponse({ status: 200, type: CrmClientProfileStatussResponse<CrmClientProfileStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades CrmClientProfileStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one crmclientprofilestatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one crmclientprofilestatus or return error" })
  @ApiResponse({ status: 200, type: CrmClientProfileStatusResponse<CrmClientProfileStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad CrmClientProfileStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get crmclientprofilestatus by ID" })
  @ApiResponse({ status: 200, type: CrmClientProfileStatusResponse<CrmClientProfileStatus> })
  @ApiResponse({ status: 404, description: "CrmClientProfileStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the crmclientprofilestatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CrmClientProfileStatusResponse<CrmClientProfileStatus>> {
    try {
      const crmclientprofilestatus = await this.service.findOne({ where: { id } });
      if (!crmclientprofilestatus) {
        throw new NotFoundException(
          "CrmClientProfileStatus no encontrado para el id solicitado"
        );
      }
      return crmclientprofilestatus;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }

...
@ApiTags("Opportunity Query")
@UseGuards(OpportunityAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("opportunities/query")
export class OpportunityQueryController {
  #logger = new Logger(OpportunityQueryController.name);

  constructor(private readonly service: OpportunityQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all opportunity with optional pagination" })
  @ApiResponse({ status: 200, type: OpportunitysResponse })
  @ApiQuery({ name: "options", required: false, type: OpportunityDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Opportunity>    
  ): Promise<OpportunitysResponse<Opportunity>> {
    try {
     
      const opportunitys = await this.service.findAll(options);
      logger.info("Retrieving all opportunity");
      return opportunitys;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find opportunity by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter opportunity', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: OpportunitysResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Opportunity no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find opportunitys with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Opportunity>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all opportunitys" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count opportunitys with conditions" })
  @ApiResponse({ status: 200, type: OpportunitysResponse<Opportunity> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one opportunity with conditions" })
  ...
  @ApiOperation({ summary: "Find one opportunity or return error" })
  @ApiResponse({ status: 200, type: OpportunityResponse<Opportunity> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the opportunity to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<OpportunityResponse<Opportunity>> {
    try {
      const opportunity = await this.service.findOne({ where: { id } });
      if (!opportunity) {
        throw new NotFoundException(

...
@ApiTags("CrmClientProfile Query")
@UseGuards(CrmClientProfileAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("crmclientprofiles/query")
export class CrmClientProfileQueryController {
  #logger = new Logger(CrmClientProfileQueryController.name);

  constructor(private readonly service: CrmClientProfileQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all crmclientprofile with optional pagination" })
  @ApiResponse({ status: 200, type: CrmClientProfilesResponse })
  @ApiQuery({ name: "options", required: false, type: CrmClientProfileDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<CrmClientProfile>    
  ): Promise<CrmClientProfilesResponse<CrmClientProfile>> {
    try {
     
      const crmclientprofiles = await this.service.findAll(options);
      logger.info("Retrieving all crmclientprofile");
  ...
  @ApiOperation({ summary: "Find crmclientprofile by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter crmclientprofile', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CrmClientProfilesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "CrmClientProfile no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find crmclientprofiles with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<CrmClientProfile>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all crmclientprofiles" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count crmclientprofiles with conditions" })
  @ApiResponse({ status: 200, type: CrmClientProfilesResponse<CrmClientProfile> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one crmclientprofile with conditions" })
  ...
  @ApiOperation({ summary: "Find one crmclientprofile or return error" })
  @ApiResponse({ status: 200, type: CrmClientProfileResponse<CrmClientProfile> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get crmclientprofile by ID" })
  @ApiResponse({ status: 200, type: CrmClientProfileResponse<CrmClientProfile> })
  @ApiResponse({ status: 404, description: "CrmClientProfile not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the crmclientprofile to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CrmClientProfileResponse<CrmClientProfile>> {
    try {
      const crmclientprofile = await this.service.findOne({ where: { id } });
      if (!crmclientprofile) {
        throw new NotFoundException(

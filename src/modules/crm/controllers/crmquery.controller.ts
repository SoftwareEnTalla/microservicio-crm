...
@Controller("crms/query")
export class CrmQueryController {
  #logger = new Logger(CrmQueryController.name);

  constructor(private readonly service: CrmQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all crm with optional pagination" })
  @ApiResponse({ status: 200, type: CrmsResponse })
  @ApiQuery({ name: "options", required: false, type: CrmDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Crm>    
  ): Promise<CrmsResponse<Crm>> {
    try {
     
      const crms = await this.service.findAll(options);
      logger.info("Retrieving all crm");
      return crms;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find crm by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter crm', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CrmsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Crm no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find crms with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Crm>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all crms" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count crms with conditions" })
  @ApiResponse({ status: 200, type: CrmsResponse<Crm> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Crms no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one crm with conditions" })
  ...
  @ApiOperation({ summary: "Find one crm or return error" })
  @ApiResponse({ status: 200, type: CrmResponse<Crm> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Crm no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get crm by ID" })
  @ApiResponse({ status: 200, type: CrmResponse<Crm> })
  @ApiResponse({ status: 404, description: "Crm not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the crm to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CrmResponse<Crm>> {
    try {
      const crm = await this.service.findOne({ where: { id } });
      if (!crm) {
        throw new NotFoundException(
          "Crm no encontrado para el id solicitado"
        );
      }
      return crm;
    } catch (error) {

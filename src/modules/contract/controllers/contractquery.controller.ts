...
@ApiTags("Contract Query")
@UseGuards(ContractAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("contracts/query")
export class ContractQueryController {
  #logger = new Logger(ContractQueryController.name);

  constructor(private readonly service: ContractQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all contract with optional pagination" })
  @ApiResponse({ status: 200, type: ContractsResponse })
  @ApiQuery({ name: "options", required: false, type: ContractDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Contract>    
  ): Promise<ContractsResponse<Contract>> {
    try {
     
      const contracts = await this.service.findAll(options);
      logger.info("Retrieving all contract");
      return contracts;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find contract by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter contract', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ContractsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Contract no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find contracts with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Contract>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all contracts" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count contracts with conditions" })
  @ApiResponse({ status: 200, type: ContractsResponse<Contract> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Contracts no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one contract with conditions" })
  ...
  @ApiOperation({ summary: "Find one contract or return error" })
  @ApiResponse({ status: 200, type: ContractResponse<Contract> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    @Query() all: Record<string, any> = {}
  ): Promise<ContractResponse<Contract> | Error> {
    try {
      const where: Record<string, any> = parseWhereParam(all);
      const entity = await this.service.findOne(where);

      if (!entity) {
        return new NotFoundException("Entidad Contract no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the contract to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ContractResponse<Contract>> {
    try {
      const contract = await this.service.findOne({ where: { id } });
      if (!contract) {
        throw new NotFoundException(
          "Contract no encontrado para el id solicitado"
        );
      }
      return contract;
    } catch (error) {

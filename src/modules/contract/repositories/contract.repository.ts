...
import { logger } from '@core/logs/logger';
  ...
  export class ContractRepository {
    ...
    async findAll(options?: FindManyOptions<Contract>): Promise<Contract[]> {
      logger.info('Ready to findAll Contract on repository:',options);
    ...
    async findById(id: string): Promise<Contract | null> {
      const tmp: FindOptionsWhere<Contract> = { id } as FindOptionsWhere<Contract>;
      logger.info('Ready to findById Contract on repository:',tmp);
    ...
    async findByField(
      ...
      logger.info('Ready to findByField Contract on repository:',options);
    ...
    async findWithPagination(
      options: FindManyOptions<Contract>,
      page: number,
      limit: number
    ): Promise<Contract[]> {
    ...
    async count(): Promise<number> {
      logger.info('Ready to count Contract on repository...');
      let result= this.repository.count();
      logger.info('Was counted  instances of Contract on repository:');
      return result;
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[Contract[], number]> {
      logger.info('Ready to findByField Contract on repository:',where);
    ...
    async findOne(options?: Record<string, any>): Promise<Contract | null> {
      if (!options || Object.keys(options).length === 0) {
        logger.warn('No conditions provided for finding Contract.');
        return null;
      }
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) Contract:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<Contract> = options as FindOptionsWhere<Contract>;
      logger.info('Ready to findOneBy Contract on repository:', tmp);
      return this.repository.findOneBy(tmp);
    }
    ...
    async findOneOrFail(where?: Record<string, any>): Promise<Contract> {
      logger.info('Ready to findOneOrFail Contract on repository:',where);
    ...
    async update(
        ...
        logger.info('Updated instance of Contract with id:  was finded on repository:', instance);

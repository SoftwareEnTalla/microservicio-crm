...
@Controller("contacts/query")
export class ContactQueryController {
  #logger = new Logger(ContactQueryController.name);

  constructor(private readonly service: ContactQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all contact with optional pagination" })
  @ApiResponse({ status: 200, type: ContactsResponse })
  @ApiQuery({ name: "options", required: false, type: ContactDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Contact>    
  ): Promise<ContactsResponse<Contact>> {
    try {
     
      const contacts = await this.service.findAll(options);
      logger.info("Retrieving all contact");
      return contacts;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find contact by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter contact', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ContactsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Contact no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find contacts with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Contact>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all contacts" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count contacts with conditions" })
  @ApiResponse({ status: 200, type: ContactsResponse<Contact> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Contacts no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one contact with conditions" })
  ...
  @ApiOperation({ summary: "Find one contact or return error" })
  @ApiResponse({ status: 200, type: ContactResponse<Contact> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    @Query() all: Record<string, any> = {}
  ): Promise<ContactResponse<Contact> | Error> {
    try {
      const where: Record<string, any> = parseWhereParam(all);
      const entity = await this.service.findOne(where);

      if (!entity) {
        return new NotFoundException("Entidad Contact no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the contact to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ContactResponse<Contact>> {
    try {
      const contact = await this.service.findOne({ where: { id } });
      if (!contact) {
        throw new NotFoundException(
          "Contact no encontrado para el id solicitado"
        );
      }
      return contact;
    } catch (error) {

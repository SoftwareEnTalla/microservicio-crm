 ...
 * Licencia: MIT
 *
 * OnIncentiveTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `incentive-type` (owner: crm). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  'incentive-type-created',
  'incentive-type-updated',
  'incentive-type-deleted',
];

@Injectable()
export class OnIncentiveTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnIncentiveTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnIncentiveTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnIncentiveTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onIncentiveTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}

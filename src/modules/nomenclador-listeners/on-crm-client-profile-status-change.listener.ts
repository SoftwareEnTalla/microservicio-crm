 ...
 * Licencia: MIT
 *
 * OnCrmClientProfileStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `crm-client-profile-status` (owner: crm). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'crm-client-profile-status';
const OWNER_BC = 'crm';
...
const TOPICS: string[] = [
  'crm-client-profile-status-created',
  'crm-client-profile-status-updated',
  'crm-client-profile-status-deleted',
];

@Injectable()
export class OnCrmClientProfileStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnCrmClientProfileStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnCrmClientProfileStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnCrmClientProfileStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onCrmClientProfileStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}

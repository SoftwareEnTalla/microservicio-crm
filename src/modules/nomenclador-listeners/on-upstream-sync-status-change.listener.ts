 ...
 * Licencia: MIT
 *
 * OnUpstreamSyncStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `upstream-sync-status` (owner: catalog). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'upstream-sync-status';
const OWNER_BC = 'catalog';
...
const TOPICS: string[] = [
  'upstream-sync-status-created',
  'upstream-sync-status-updated',
  'upstream-sync-status-deleted',
];

@Injectable()
export class OnUpstreamSyncStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnUpstreamSyncStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnUpstreamSyncStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnUpstreamSyncStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onUpstreamSyncStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}

 ...
 * Licencia: MIT
 *
 * OnPaymentMilestoneStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `payment-milestone-status` (owner: crm). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'payment-milestone-status';
const OWNER_BC = 'crm';
...
const TOPICS: string[] = [
  'payment-milestone-status-created',
  'payment-milestone-status-updated',
  'payment-milestone-status-deleted',
];

@Injectable()
export class OnPaymentMilestoneStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnPaymentMilestoneStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnPaymentMilestoneStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnPaymentMilestoneStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onPaymentMilestoneStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}

 ...
 * Licencia: MIT
 *
 * OnBillingCycleChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `billing-cycle` (owner: crm). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  ...
  'billing-cycle-deleted',
];

@Injectable()
export class OnBillingCycleChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnBillingCycleChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnBillingCycleChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnBillingCycleChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onBillingCycleChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}

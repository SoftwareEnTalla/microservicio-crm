 ...
 * Licencia: MIT
 *
 * OnGlobalPaymentRuleChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `global-payment-rule` (owner: crm). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'global-payment-rule';
const OWNER_BC = 'crm';
...
const TOPICS: string[] = [
  'global-payment-rule-created',
  'global-payment-rule-updated',
  'global-payment-rule-deleted',
];

@Injectable()
export class OnGlobalPaymentRuleChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnGlobalPaymentRuleChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnGlobalPaymentRuleChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnGlobalPaymentRuleChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onGlobalPaymentRuleChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}

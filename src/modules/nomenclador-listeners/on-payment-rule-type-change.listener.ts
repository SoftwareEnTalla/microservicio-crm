 ...
 * Licencia: MIT
 *
 * OnPaymentRuleTypeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `payment-rule-type` (owner: crm). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'payment-rule-type';
const OWNER_BC = 'crm';
...
const TOPICS: string[] = [
  'payment-rule-type-created',
  'payment-rule-type-updated',
  'payment-rule-type-deleted',
];

@Injectable()
export class OnPaymentRuleTypeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnPaymentRuleTypeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnPaymentRuleTypeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnPaymentRuleTypeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onPaymentRuleTypeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}

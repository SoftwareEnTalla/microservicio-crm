...
import { IncentiveDto } from "../dtos/all-dto";
...
@ApiTags("Incentive Query")
@UseGuards(IncentiveAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("incentives/query")
export class IncentiveQueryController {
  #logger = new Logger(IncentiveQueryController.name);

  constructor(private readonly service: IncentiveQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all incentive with optional pagination" })
  @ApiResponse({ status: 200, type: IncentivesResponse })
  @ApiQuery({ name: "options", required: false, type: IncentiveDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Incentive>    
  ): Promise<IncentivesResponse<Incentive>> {
    try {
     
      const incentives = await this.service.findAll(options);
      logger.info("Retrieving all incentive");
      return incentives;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find incentive by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter incentive', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: IncentivesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Incentive no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find incentives with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Incentive>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all incentives" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count incentives with conditions" })
  @ApiResponse({ status: 200, type: IncentivesResponse<Incentive> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Incentives no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one incentive with conditions" })
  ...
  @ApiOperation({ summary: "Find one incentive or return error" })
  @ApiResponse({ status: 200, type: IncentiveResponse<Incentive> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    @Query() all: Record<string, any> = {}
  ): Promise<IncentiveResponse<Incentive> | Error> {
    try {
      const where: Record<string, any> = parseWhereParam(all);
      const entity = await this.service.findOne(where);

      if (!entity) {
        return new NotFoundException("Entidad Incentive no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get incentive by ID" })
  @ApiResponse({ status: 200, type: IncentiveResponse<Incentive> })
  @ApiResponse({ status: 404, description: "Incentive not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the incentive to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<IncentiveResponse<Incentive>> {
    try {
      const incentive = await this.service.findOne({ where: { id } });
      if (!incentive) {
        throw new NotFoundException(
          "Incentive no encontrado para el id solicitado"
        );
      }
      return incentive;
    } catch (error) {

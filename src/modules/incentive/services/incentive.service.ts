...
export class IncentiveService {
  ...
  // Métodos delegados
  async findAll(options?: any): Promise<Incentive[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<Incentive[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<Incentive[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<Incentive | null> {
  ...
  async update(id: string, partialEntity: Partial<Incentive>): Promise<Incentive | null> {

...
@Injectable()
export class MilestoneStatusLogQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(MilestoneStatusLogQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: MilestoneStatusLogQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${MilestoneStatusLog.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const milestonestatuslogs = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de milestonestatuslogs obtenido con éxito",
        data: milestonestatuslogs,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
          milestonestatuslogs.length
        ),
        count: milestonestatuslogs.length,
      };
    } catch (error) {
      // Imprimir error
      logger.error(error);
  ...
  async findById(id: string): Promise<MilestoneStatusLogResponse<MilestoneStatusLog>> {
    try {
      const milestonestatuslog = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "MilestoneStatusLogs no encontrados para la propiedad y valor especificado"
        );
      // Devolver milestonestatuslog
      return {
        ok: true,
        message: "MilestoneStatusLogs obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<MilestoneStatusLogResponse<MilestoneStatusLog>> {
    try {

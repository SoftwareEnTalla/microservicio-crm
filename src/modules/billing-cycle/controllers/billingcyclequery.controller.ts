...
@ApiTags("BillingCycle Query")
@UseGuards(BillingCycleAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("billingcycles/query")
export class BillingCycleQueryController {
  #logger = new Logger(BillingCycleQueryController.name);

  constructor(private readonly service: BillingCycleQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all billingcycle with optional pagination" })
  @ApiResponse({ status: 200, type: BillingCyclesResponse })
  @ApiQuery({ name: "options", required: false, type: BillingCycleDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<BillingCycle>    
  ): Promise<BillingCyclesResponse<BillingCycle>> {
    try {
     
      const billingcycles = await this.service.findAll(options);
      logger.info("Retrieving all billingcycle");
  ...
  @ApiOperation({ summary: "Find billingcycle by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter billingcycle', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: BillingCyclesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "BillingCycle no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find billingcycles with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<BillingCycle>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all billingcycles" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count billingcycles with conditions" })
  @ApiResponse({ status: 200, type: BillingCyclesResponse<BillingCycle> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades BillingCycles no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one billingcycle with conditions" })
  ...
  @ApiOperation({ summary: "Find one billingcycle or return error" })
  @ApiResponse({ status: 200, type: BillingCycleResponse<BillingCycle> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad BillingCycle no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get billingcycle by ID" })
  @ApiResponse({ status: 200, type: BillingCycleResponse<BillingCycle> })
  @ApiResponse({ status: 404, description: "BillingCycle not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the billingcycle to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<BillingCycleResponse<BillingCycle>> {
    try {
      const billingcycle = await this.service.findOne({ where: { id } });
      if (!billingcycle) {
        throw new NotFoundException(
          "BillingCycle no encontrado para el id solicitado"
        );
      }
      return billingcycle;
    } catch (error) {

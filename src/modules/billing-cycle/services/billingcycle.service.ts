...
export class BillingCycleService {
  constructor(private readonly repository: BillingCycleRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<BillingCycle[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<BillingCycle[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<BillingCycle[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<BillingCycle | null> {
  ...
  async update(id: string, partialEntity: Partial<BillingCycle>): Promise<BillingCycle | null> {

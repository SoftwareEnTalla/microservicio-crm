...
@Injectable()
export class ActivityQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(ActivityQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: ActivityQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${Activity.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const activitys = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de activitys obtenido con éxito",
        data: activitys,
  ...
  async findById(id: string): Promise<ActivityResponse<Activity>> {
    try {
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "Activitys no encontrados para la propiedad y valor especificado"
        );
      // Devolver activity
      return {
        ok: true,
        message: "Activitys obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<ActivityResponse<Activity>> {
    try {

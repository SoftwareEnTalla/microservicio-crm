...
@ApiTags("GlobalPaymentRule Query")
@UseGuards(GlobalPaymentRuleAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("globalpaymentrules/query")
export class GlobalPaymentRuleQueryController {
  #logger = new Logger(GlobalPaymentRuleQueryController.name);

  constructor(private readonly service: GlobalPaymentRuleQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all globalpaymentrule with optional pagination" })
  @ApiResponse({ status: 200, type: GlobalPaymentRulesResponse })
  @ApiQuery({ name: "options", required: false, type: GlobalPaymentRuleDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<GlobalPaymentRule>    
  ): Promise<GlobalPaymentRulesResponse<GlobalPaymentRule>> {
    try {
     
      const globalpaymentrules = await this.service.findAll(options);
      logger.info("Retrieving all globalpaymentrule");
  ...
  @ApiOperation({ summary: "Find globalpaymentrule by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter globalpaymentrule', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: GlobalPaymentRulesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "GlobalPaymentRule no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find globalpaymentrules with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<GlobalPaymentRule>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all globalpaymentrules" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count globalpaymentrules with conditions" })
  @ApiResponse({ status: 200, type: GlobalPaymentRulesResponse<GlobalPaymentRule> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades GlobalPaymentRules no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one globalpaymentrule with conditions" })
  ...
  @ApiOperation({ summary: "Find one globalpaymentrule or return error" })
  @ApiResponse({ status: 200, type: GlobalPaymentRuleResponse<GlobalPaymentRule> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad GlobalPaymentRule no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get globalpaymentrule by ID" })
  @ApiResponse({ status: 200, type: GlobalPaymentRuleResponse<GlobalPaymentRule> })
  @ApiResponse({ status: 404, description: "GlobalPaymentRule not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the globalpaymentrule to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<GlobalPaymentRuleResponse<GlobalPaymentRule>> {
    try {
      const globalpaymentrule = await this.service.findOne({ where: { id } });
      if (!globalpaymentrule) {
        throw new NotFoundException(
          "GlobalPaymentRule no encontrado para el id solicitado"
        );
      }
      return globalpaymentrule;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }

...
@ApiTags("PaymentRuleType Query")
@UseGuards(PaymentRuleTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("paymentruletypes/query")
export class PaymentRuleTypeQueryController {
  #logger = new Logger(PaymentRuleTypeQueryController.name);

  constructor(private readonly service: PaymentRuleTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all paymentruletype with optional pagination" })
  @ApiResponse({ status: 200, type: PaymentRuleTypesResponse })
  @ApiQuery({ name: "options", required: false, type: PaymentRuleTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PaymentRuleType>    
  ): Promise<PaymentRuleTypesResponse<PaymentRuleType>> {
    try {
     
      const paymentruletypes = await this.service.findAll(options);
      logger.info("Retrieving all paymentruletype");
  ...
  @ApiOperation({ summary: "Find paymentruletype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter paymentruletype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PaymentRuleTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PaymentRuleType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find paymentruletypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PaymentRuleType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all paymentruletypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count paymentruletypes with conditions" })
  @ApiResponse({ status: 200, type: PaymentRuleTypesResponse<PaymentRuleType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PaymentRuleTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one paymentruletype with conditions" })
  ...
  @ApiOperation({ summary: "Find one paymentruletype or return error" })
  @ApiResponse({ status: 200, type: PaymentRuleTypeResponse<PaymentRuleType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PaymentRuleType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get paymentruletype by ID" })
  @ApiResponse({ status: 200, type: PaymentRuleTypeResponse<PaymentRuleType> })
  @ApiResponse({ status: 404, description: "PaymentRuleType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the paymentruletype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PaymentRuleTypeResponse<PaymentRuleType>> {
    try {
      const paymentruletype = await this.service.findOne({ where: { id } });
      if (!paymentruletype) {
        throw new NotFoundException(
          "PaymentRuleType no encontrado para el id solicitado"
        );
      }

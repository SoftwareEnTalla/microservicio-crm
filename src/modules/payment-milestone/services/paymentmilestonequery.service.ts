...
@Injectable()
export class PaymentMilestoneQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(PaymentMilestoneQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: PaymentMilestoneQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${PaymentMilestone.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const paymentmilestones = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de paymentmilestones obtenido con éxito",
        ...
        count: paymentmilestones.length,
      };
    } catch (error) {
      // Imprimir error
      logger.error(error);
  ...
  async findById(id: string): Promise<PaymentMilestoneResponse<PaymentMilestone>> {
    try {
      const paymentmilestone = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "PaymentMilestones no encontrados para la propiedad y valor especificado"
        );
      // Devolver paymentmilestone
      return {
        ok: true,
        message: "PaymentMilestones obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<PaymentMilestoneResponse<PaymentMilestone>> {
    try {

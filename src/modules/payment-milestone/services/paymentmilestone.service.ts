...
export class PaymentMilestoneService {
  constructor(private readonly repository: PaymentMilestoneRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<PaymentMilestone[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<PaymentMilestone[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<PaymentMilestone[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<PaymentMilestone | null> {
  ...
  async update(id: string, partialEntity: Partial<PaymentMilestone>): Promise<PaymentMilestone | null> {

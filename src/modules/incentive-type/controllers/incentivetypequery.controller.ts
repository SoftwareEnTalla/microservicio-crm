...
@ApiTags("IncentiveType Query")
@UseGuards(IncentiveTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("incentivetypes/query")
export class IncentiveTypeQueryController {
  #logger = new Logger(IncentiveTypeQueryController.name);

  constructor(private readonly service: IncentiveTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all incentivetype with optional pagination" })
  @ApiResponse({ status: 200, type: IncentiveTypesResponse })
  @ApiQuery({ name: "options", required: false, type: IncentiveTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<IncentiveType>    
  ): Promise<IncentiveTypesResponse<IncentiveType>> {
    try {
     
      const incentivetypes = await this.service.findAll(options);
      logger.info("Retrieving all incentivetype");
  ...
  @ApiOperation({ summary: "Find incentivetype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter incentivetype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: IncentiveTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "IncentiveType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find incentivetypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<IncentiveType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all incentivetypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count incentivetypes with conditions" })
  @ApiResponse({ status: 200, type: IncentiveTypesResponse<IncentiveType> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one incentivetype with conditions" })
  ...
  @ApiOperation({ summary: "Find one incentivetype or return error" })
  @ApiResponse({ status: 200, type: IncentiveTypeResponse<IncentiveType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get incentivetype by ID" })
  @ApiResponse({ status: 200, type: IncentiveTypeResponse<IncentiveType> })
  @ApiResponse({ status: 404, description: "IncentiveType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the incentivetype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<IncentiveTypeResponse<IncentiveType>> {
    try {
      const incentivetype = await this.service.findOne({ where: { id } });
      if (!incentivetype) {
        throw new NotFoundException(

...
@ApiTags("PersonType Query")
@UseGuards(PersonTypeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("persontypes/query")
export class PersonTypeQueryController {
  #logger = new Logger(PersonTypeQueryController.name);

  constructor(private readonly service: PersonTypeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all persontype with optional pagination" })
  @ApiResponse({ status: 200, type: PersonTypesResponse })
  @ApiQuery({ name: "options", required: false, type: PersonTypeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PersonType>    
  ): Promise<PersonTypesResponse<PersonType>> {
    try {
     
      const persontypes = await this.service.findAll(options);
      logger.info("Retrieving all persontype");
      return persontypes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find persontype by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter persontype', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PersonTypesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PersonType no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find persontypes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PersonType>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all persontypes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count persontypes with conditions" })
  @ApiResponse({ status: 200, type: PersonTypesResponse<PersonType> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PersonTypes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one persontype with conditions" })
  ...
  @ApiOperation({ summary: "Find one persontype or return error" })
  @ApiResponse({ status: 200, type: PersonTypeResponse<PersonType> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PersonType no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get persontype by ID" })
  @ApiResponse({ status: 200, type: PersonTypeResponse<PersonType> })
  @ApiResponse({ status: 404, description: "PersonType not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the persontype to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PersonTypeResponse<PersonType>> {
    try {
      const persontype = await this.service.findOne({ where: { id } });
      if (!persontype) {
        throw new NotFoundException(
          "PersonType no encontrado para el id solicitado"
        );
      }
      return persontype;
    } catch (error) {

...
import { PaymentMilestoneStatusDto } from "../dtos/all-dto";
...
@ApiTags("PaymentMilestoneStatus Query")
@UseGuards(PaymentMilestoneStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("paymentmilestonestatuss/query")
export class PaymentMilestoneStatusQueryController {
  #logger = new Logger(PaymentMilestoneStatusQueryController.name);

  constructor(private readonly service: PaymentMilestoneStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all paymentmilestonestatus with optional pagination" })
  @ApiResponse({ status: 200, type: PaymentMilestoneStatussResponse })
  @ApiQuery({ name: "options", required: false, type: PaymentMilestoneStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<PaymentMilestoneStatus>    
  ): Promise<PaymentMilestoneStatussResponse<PaymentMilestoneStatus>> {
    try {
     
      const paymentmilestonestatuss = await this.service.findAll(options);
      logger.info("Retrieving all paymentmilestonestatus");
  ...
  @ApiOperation({ summary: "Find paymentmilestonestatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter paymentmilestonestatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: PaymentMilestoneStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "PaymentMilestoneStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find paymentmilestonestatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<PaymentMilestoneStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all paymentmilestonestatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count paymentmilestonestatuss with conditions" })
  @ApiResponse({ status: 200, type: PaymentMilestoneStatussResponse<PaymentMilestoneStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades PaymentMilestoneStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one paymentmilestonestatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one paymentmilestonestatus or return error" })
  @ApiResponse({ status: 200, type: PaymentMilestoneStatusResponse<PaymentMilestoneStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad PaymentMilestoneStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get paymentmilestonestatus by ID" })
  @ApiResponse({ status: 200, type: PaymentMilestoneStatusResponse<PaymentMilestoneStatus> })
  @ApiResponse({ status: 404, description: "PaymentMilestoneStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the paymentmilestonestatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<PaymentMilestoneStatusResponse<PaymentMilestoneStatus>> {
    try {
      const paymentmilestonestatus = await this.service.findOne({ where: { id } });
      if (!paymentmilestonestatus) {
        throw new NotFoundException(
          "PaymentMilestoneStatus no encontrado para el id solicitado"
        );
      }
      return paymentmilestonestatus;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }

...
@ApiTags("ContractStatusLog Query")
@UseGuards(ContractStatusLogAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("contractstatuslogs/query")
export class ContractStatusLogQueryController {
  #logger = new Logger(ContractStatusLogQueryController.name);

  constructor(private readonly service: ContractStatusLogQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all contractstatuslog with optional pagination" })
  @ApiResponse({ status: 200, type: ContractStatusLogsResponse })
  @ApiQuery({ name: "options", required: false, type: ContractStatusLogDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<ContractStatusLog>    
  ): Promise<ContractStatusLogsResponse<ContractStatusLog>> {
    try {
     
      const contractstatuslogs = await this.service.findAll(options);
      logger.info("Retrieving all contractstatuslog");
  ...
  @ApiOperation({ summary: "Find contractstatuslog by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter contractstatuslog', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ContractStatusLogsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "ContractStatusLog no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find contractstatuslogs with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<ContractStatusLog>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all contractstatuslogs" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count contractstatuslogs with conditions" })
  @ApiResponse({ status: 200, type: ContractStatusLogsResponse<ContractStatusLog> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one contractstatuslog with conditions" })
  ...
  @ApiOperation({ summary: "Find one contractstatuslog or return error" })
  @ApiResponse({ status: 200, type: ContractStatusLogResponse<ContractStatusLog> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get contractstatuslog by ID" })
  @ApiResponse({ status: 200, type: ContractStatusLogResponse<ContractStatusLog> })
  @ApiResponse({ status: 404, description: "ContractStatusLog not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the contractstatuslog to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ContractStatusLogResponse<ContractStatusLog>> {
    try {
      const contractstatuslog = await this.service.findOne({ where: { id } });

...
import { logger } from '@core/logs/logger';
  ...
  export class QuoteRepository {
    ...
    async findAll(options?: FindManyOptions<Quote>): Promise<Quote[]> {
      logger.info('Ready to findAll Quote on repository:',options);
    ...
    async findById(id: string): Promise<Quote | null> {
      const tmp: FindOptionsWhere<Quote> = { id } as FindOptionsWhere<Quote>;
      logger.info('Ready to findById Quote on repository:',tmp);
    ...
    async findByField(
      ...
      logger.info('Ready to findByField Quote on repository:',options);
    ...
    async count(): Promise<number> {
      logger.info('Ready to count Quote on repository...');
      let result= this.repository.count();
      logger.info('Was counted  instances of Quote on repository:');
      return result;
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[Quote[], number]> {
      logger.info('Ready to findByField Quote on repository:',where);
    ...
    async findOne(options?: Record<string, any>): Promise<Quote | null> {
      if (!options || Object.keys(options).length === 0) {
        logger.warn('No conditions provided for finding Quote.');
        return null;
      }
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) Quote:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<Quote> = options as FindOptionsWhere<Quote>;
      logger.info('Ready to findOneBy Quote on repository:', tmp);
      return this.repository.findOneBy(tmp);
    }
    ...
    async findOneOrFail(where?: Record<string, any>): Promise<Quote> {
      logger.info('Ready to findOneOrFail Quote on repository:',where);
    ...
    async update(
        ...
        logger.info('Updated instance of Quote with id:  was finded on repository:', instance);

...
import { QuoteDto } from "../dtos/all-dto";
...
@Controller("quotes/query")
export class QuoteQueryController {
  #logger = new Logger(QuoteQueryController.name);

  constructor(private readonly service: QuoteQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all quote with optional pagination" })
  @ApiResponse({ status: 200, type: QuotesResponse })
  @ApiQuery({ name: "options", required: false, type: QuoteDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Quote>    
  ): Promise<QuotesResponse<Quote>> {
    try {
     
      const quotes = await this.service.findAll(options);
      logger.info("Retrieving all quote");
      return quotes;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find quote by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter quote', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: QuotesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Quote no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find quotes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Quote>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all quotes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count quotes with conditions" })
  @ApiResponse({ status: 200, type: QuotesResponse<Quote> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Quotes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one quote with conditions" })
  ...
  @ApiOperation({ summary: "Find one quote or return error" })
  @ApiResponse({ status: 200, type: QuoteResponse<Quote> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Quote no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the quote to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<QuoteResponse<Quote>> {
    try {
      const quote = await this.service.findOne({ where: { id } });
      if (!quote) {
        throw new NotFoundException(
          "Quote no encontrado para el id solicitado"
        );
      }
      return quote;
    } catch (error) {
